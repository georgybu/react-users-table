            ...
            <form className={styles.userForm} onSubmit={handleSubmit(onSubmit)}>
                <div className={styles.formField}>
                    <input name="email" placeholder="Email"
                           ref={register({pattern: {value: emailPattern, message: 'Invalid Email'}, required: true})}/>
                    {errors.email && <span
                      className={styles.error}>{errors.email?.type === 'pattern' ? 'Invalid email.' : 'Email is required.'}</span>}
                </div>
                <div className={styles.formField}>
